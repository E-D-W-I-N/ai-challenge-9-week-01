...
class Session:
    ...
    под колонкой считает долю уникальных ответов, а судье отдаёт все ответы.
...
class Scenario:
    ...
    Подсказка ведущему, и только ему: судье она не отправляется. Поэтому
    писать сюда можно всё, включая эталонный ответ и предсказание, какая
    колонка сядет в ловушку.
    """
    ...
    label, note, отличающие её параметры, метрики и полный ответ. watch_for
    судье не отправляется: это подсказка ведущему, а не данные для вердикта.
